	...
	return (
		<Layout>
			<Heading title={'Описание учащегося'}>
				<span className="text-base text-gray-500">{student?.surname} {student?.name} {student?.secondName}</span>
			</Heading>
			<table className="min-w-full border-gray-300">
				...
				<tbody>
				{student?.medicalCertificates?.map(({ id, startDate, finishDate }) => (
					<tr
					...
					>
						<td>{student?.name}</td>
						<td className=" p-2">{format(new Date(startDate), 'dd.MM.yyyy')}</td>
						<td className=" p-2">
							{format(new Date(finishDate), 'dd.MM.yyyy')}
						</td>
						<td>1</td>
						<td>2</td>
						<td>{student?.medicalCertificates.map(item => item.healthGroupId)}</td>
					</tr>
				))}
				</tbody>
			</table>
